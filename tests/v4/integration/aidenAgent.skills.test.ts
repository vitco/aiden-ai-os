/**
 * Phase 10 skills integration test. Live Groq when available;
 ...
 *
 * Test creates a one-off skill in a tmp directory so we don't
 * depend on bundled skill names that may change.
 */
...
describe.skipIf(!GROQ_KEY)('AidenAgent + Phase 10 skills (Groq)', () => {
  it('uses skills_list and skill_view via progressive disclosure', async () => {
    ...
    // Plant a one-off skill with a uniquely-marked body so we can
    // verify the agent surfaces its content.
